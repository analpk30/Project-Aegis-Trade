...
try:
    ...
    _app_root_env = Path(__file__).resolve().parents[1] / ".env"
    _repo_root_env = Path(__file__).resolve().parents[2] / ".env"
    if _app_root_env.exists():
        load_dotenv(_app_root_env)
...
from ai_engine import (
    generate_mifid_justification,
    interpret_bafin_rules,
    get_engine_status,
    set_engine_mode,
    generate_ai_ideas_from_history,
)
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
            ...
            self.send_json(
                {
                    "activePersona": store.active_persona,
                    "activeUser": store.active_user,
                    "config": PERSONA_CONFIG.get(
                        store.active_persona, PERSONA_CONFIG["Trader"]
                    ),
        ...
        if path == "/api/search":
            ...
            matching_orders = [
                ...
                if q in o["id"].lower()
                or q in o["instrument"].lower()
                or q in o["clientName"].lower()
            ]
            matching_clients = [
                c
            ...
            matching_bafin = [
                b
                for b in store.bafin_announcements
                if q in b["title"].lower() or q in b["summary"].lower()
            ]
            matching_cases = [
                fc
                for fc in SEEDED_FINE_CASES
                if q in fc["caseName"].lower() or q in fc["category"].lower()
            ]
            ...
            self.send_json(
                {
                    ...
                    "paths": {
                        "/api/auth/persona": {
                            "get": {"summary": "Get current persona"},
                            "post": {"summary": "Switch persona role"},
                        },
                        "/api/orders": {
                            "get": {"summary": "Get active blotter orders"}
                        },
                        "/api/orders/{id}/autopilot": {
                            "post": {"summary": "Stream MiFID II justification"}
                        },
                        "/api/precrime/score": {
                            "post": {
                                "summary": "Compute vector similarity to historical fine cases"
                            }
                        },
                        "/api/bafin/interpret": {
                            "post": {"summary": "BaFin rulebook RAG query"}
                        },
                    },
                }
            )
        ...
        if path == "/api/stream":
            ...
            try:
                avg_score = round(
                    sum(o["guardianScore"] for o in store.orders)
                    / max(1, len(store.orders))
                )
                pulse_msg = {
                    "type": "pulse",
    ...
    def do_POST(self):
        ...
        if path == "/api/engine/benchmark":
            ...
            order = next(
                (o for o in store.orders if o["id"] == test_order_id), store.orders[0]
            )
            ...
            self.send_json(
                {
                    ...
                    "latencyDeltaMs": abs(
                        primary_res["latencyMs"] - fallback_res["latencyMs"]
                    ),
                }
            )
            return
        ...
        if re.match(r"^/api/orders/[^/]+/autopilot$", path):
            ...
            client = next(
                (c for c in store.clients if c["id"] == order["clientId"]),
                store.clients[0],
            )
            ...
            # Start message
            self.wfile.write(
                f"data: {json.dumps({'type': 'start', 'orderId': order_id, 'guardianScore': computed['score'], 'breakdown': computed['breakdown']})}\n\n".encode(
                    "utf-8"
                )
            )
            self.wfile.flush()
            ...
            # Stream chunk
            self.wfile.write(
                f"data: {json.dumps({'type': 'chunk', 'text': llm_result['text']})}\n\n".encode(
                    "utf-8"
                )
            )
            self.wfile.flush()
            ...
            self.wfile.write(
                f"data: {json.dumps({'type': 'complete', 'order': order, 'auditEntry': audit_entry})}\n\n".encode(
                    "utf-8"
                )
            )
            self.wfile.flush()
        ...
        if path == "/api/precrime/score":
            ...
            client = next(
                (c for c in store.clients if c["id"] == client_id), store.clients[0]
            )
        ...
        if re.match(r"^/api/ideas/[^/]+/send-to-blotter$", path):
            ...
            self.send_json(
                {"success": True, "newOrder": new_order, "auditEntry": audit_entry}
            )
            return
        ...
        if path == "/api/bafin/interpret":
            ...
            interpretation = interpret_bafin_rules(
                query, store.bafin_announcements, chat_session_id
            )
            ...
            matching = [
                a for a in store.bafin_announcements if a["id"] in retrieved_ids
            ]
        ...
        if path == "/api/risk/simulate-shock" or path == "/api/risk/simulate-reset":
            ...
            try:
                ...
                self.send_json(
                    {"error": f"anomaly engine unavailable: {e}"}, status=503
                )
            return
        ...
        if path == "/api/risk/anomalies/narrate":
            ...
            if anomaly_id:
                anomaly = next(
                    (a for a in store.anomalies if a["id"] == anomaly_id), None
                )
        ...
        if path == "/api/audit/export":
            ...
            self.send_header(
                "Content-Disposition",
                'attachment; filename="Project_Guardian_BaFin_Audit_Report.pdf"',
            )
            self.end_headers()
